...
#family: model to use, gaussian, poisson
#estimation.method: Bayesian or MLE
DRAgeneLevel<-function(eset,do.log2=TRUE,do.restand=TRUE,filterLowCount=TRUE,filterBy='control',count.cutoff=16,family=gaussian,estimation.method='Bayesian'){
...
#	DR<-plyr::ddply(fData(eset),'gene',function(x){
#				apply(x,2,function(y){
#							y<-sort(unique(y[y!=''|!is.na(y)]))
#							paste(y,collapse=';')
#						})
#			})
...
#i=1
	...
	for(i in 1:length(condition)){
		...
		dr<-plyr::ddply(d,'gene','combRowEvid.2grps',comp=comp,family=family,method=estimation.method,n.iter=5000,nitt=25000,burnin=5000,thin=1,logTransformed=do.log2,restand=do.restand,pseudoCount=0,pooling='partial')
	...
	col.sel<-c('n.sh_sgRNAs.passFilter','log2FC','z','pval','pvalMCMC','FDR','B','B.sd','AveSignal')
	...
	DR<-DR[,
			...
			DR<-DR[order(DR[,grep('^pval\\.',names(DR))]),]
